...
RoboCupSSLDebugClient::RoboCupSSLDebugClient(int port,
                     string net_address,
...
RoboCupSSLDebugClient::~RoboCupSSLDebugClient()
{
  delete[] in_buffer;
}

void RoboCupSSLDebugClient::close() {
  mc.close();
}

bool RoboCupSSLDebugClient::open(bool blocking) {
  close();
...
bool RoboCupSSLDebugClient::receive(sslDebug_Data & packet) {
  Net::Address src;
  int r=0;
