import rospy,sys
from utils.geometry import Vector2D
from utils.functions import *
from krssg_ssl_msgs.msg import point_2d
from krssg_ssl_msgs.msg import BeliefState
from krssg_ssl_msgs.msg import gr_Commands
from krssg_ssl_msgs.msg import gr_Robot_Command
from krssg_ssl_msgs.msg import BeliefState
from role import  GoToBall, GoToPoint,random1
from multiprocessing import Process
from kubs import kubs
from krssg_ssl_msgs.srv import bsServer
from math import atan2,pi
from utils.functions import *
pub = rospy.Publisher('/grsim_data',gr_Commands,queue_size=1000)
import multiprocessing
import threading

import memcache
shared = memcache.Client(['127.0.0.1:11211'],debug=True)



def function1(id_,state,pub):
	kub = kubs.kubs(id_,state,pub)
	kub.update_state(state)
	#print(kub.kubs_id)
	g_fsm = random1.Random1()
	# g_fsm = GoToPoint.GoToPoint()
	g_fsm.add_kub(kub)
	# g_fsm.add_point(point=kub.state.ballPos,orient=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x-3000)))
	g_fsm.add_theta(theta=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x+3000)))
	# g_fsm.as_graphviz()
	# g_fsm.write_diagram_png()
	# print('something before spin')
	g_fsm.spin()
	# 
def function2(id_,state,pub):
	kub = kubs.kubs(id_,state,pub)
	kub.update_state(state)
	#print(kub.kubs_id)
	g_fsm = random1.Random1()
	# g_fsm = GoToPoint.GoToPoint()
	g_fsm.add_kub(kub)
	# g_fsm.add_point(point=kub.state.ballPos,orient=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x-3000)))
	g_fsm.add_theta(theta=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x+3000)))
	# g_fsm.as_graphviz()
	# g_fsm.write_diagram_png()
	# print('something before spin')
	g_fsm.spin()
	# 
def function3(id_,state,pub):
	kub = kubs.kubs(id_,state,pub)
	kub.update_state(state)
	#print(kub.kubs_id)
	g_fsm = random1.Random2()
	# g_fsm = GoToPoint.GoToPoint()
	g_fsm.add_kub(kub)
	# g_fsm.add_point(point=kub.state.ballPos,orient=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x-3000)))
	g_fsm.add_theta(theta=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x+3000)))
	# g_fsm.as_graphviz()
	# g_fsm.write_diagram_png()
	# print('something before spin')
	g_fsm.spin()
	# 
def function4(id_,state,pub):
	kub = kubs.kubs(id_,state,pub)
	kub.update_state(state)
	#print(kub.kubs_id)
	g_fsm = random1.Random2()
	# g_fsm = GoToPoint.GoToPoint()
	g_fsm.add_kub(kub)
	# g_fsm.add_point(point=kub.state.ballPos,orient=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x-3000)))
	g_fsm.add_theta(theta=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x+3000)))
	g_fsm.spin()
	# 
def function5(id_,state,pub):
	kub = kubs.kubs(id_,state,pub)
	kub.update_state(state)
	#print(kub.kubs_id)
	g_fsm = random1.Random3()
	# g_fsm = GoToPoint.GoToPoint()
	g_fsm.add_kub(kub)
	# g_fsm.add_point(point=kub.state.ballPos,orient=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x-3000)))
	g_fsm.add_theta(theta=normalize_angle(pi+atan2(state.ballPos.y,state.ballPos.x+3000)))
	g_fsm.spin()
	# 



# rospy.Subscriber('/belief_state', BeliefState, BS_callback, queue_size=1000)
def main1(process_id):
	pub = rospy.Publisher('/grsim_data',gr_Commands,queue_size=1000)
	rospy.init_node('node' + str(process_id),anonymous=False)
	while True:
		state = None
		#state=shared.get('state')
		rospy.wait_for_service('bsServer',)
		getState = rospy.ServiceProxy('bsServer',bsServer)
		try:
			state = getState(state)
		except rospy.ServiceException, e:
			print("chutiya")		
		if state:
			print('lasknfcjscnajnstate',state.stateB.homePos)
			function1(1,state.stateB,pub)
			#function5(1,state.stateB)
			print('chal ja')
			# break
	#rospy.spin()	
def main2(process_id):
	pub = rospy.Publisher('/grsim_data',gr_Commands,queue_size=1000)
	rospy.init_node('node' + str(process_id),anonymous=False)
	while True:
		state = None
		#state=shared.get('state')
		rospy.wait_for_service('bsServer',)
		getState = rospy.ServiceProxy('bsServer',bsServer)
		try:
			state = getState(state)
		except rospy.ServiceException, e:
			print("chutiya")		
		if state:
			print('lasknfcjscnajnstate',state.stateB.homePos)
			function2(2,state.stateB,pub)
			#function5(1,state.stateB)
			print('chal ja')
			# break
	#rospy.spin()	
def main3(process_id):
	pub = rospy.Publisher('/grsim_data',gr_Commands,queue_size=1000)
	rospy.init_node('node' + str(process_id),anonymous=False)
	while True:
		state = None
		#state=shared.get('state')
		rospy.wait_for_service('bsServer',)
		getState = rospy.ServiceProxy('bsServer',bsServer)
		try:
			state = getState(state)
		except rospy.ServiceException, e:
			print("chutiya")		
		if state:
			print('lasknfcjscnajnstate',state.stateB.homePos)
			function3(3,state.stateB,pub)
			#function5(1,state.stateB)
			print('chal ja')
			# break
	#rospy.spin()
def main4(process_id):
	pub = rospy.Publisher('/grsim_data',gr_Commands,queue_size=1000)
	rospy.init_node('node' + str(process_id),anonymous=False)
	while True:
		state = None
		#state=shared.get('state')
		rospy.wait_for_service('bsServer',)
		getState = rospy.ServiceProxy('bsServer',bsServer)
		try:
			state = getState(state)
		except rospy.ServiceException, e:
			print("chutiya")		
		if state:
			print('lasknfcjscnajnstate',state.stateB.homePos)
			function4(4,state.stateB,pub)
			#function5(1,state.stateB)
			print('chal ja')
			# break
	#rospy.spin()	

def main5(process_id):
	pub = rospy.Publisher('/grsim_data',gr_Commands,queue_size=1000)
	rospy.init_node('node' + str(process_id),anonymous=False)
	while True:
		state = None
		#state=shared.get('state')
		rospy.wait_for_service('bsServer',)
		getState = rospy.ServiceProxy('bsServer',bsServer)
		try:
			state = getState(state)
		except rospy.ServiceException, e:
			print("chutiya")		
		if state:
			print('lasknfcjscnajnstate',state.stateB.homePos)
			function5(5,state.stateB,pub)
			#function5(1,state.stateB)
			print('chal ja')
			# break
	#rospy.spin()

p1 = multiprocessing.Process(target=main1, args=(1,))
p2 = multiprocessing.Process(target=main2, args=(2,))
p3 = multiprocessing.Process(target=main3, args=(3,))
p4 = multiprocessing.Process(target=main4, args=(4,))
p5 = multiprocessing.Process(target=main5, args=(5,))
p1.start()
p2.start()
p3.start()
p4.start()
p5.start()
p1.join()
p2.join()
p3.join()
p4.join()
p5.join()


