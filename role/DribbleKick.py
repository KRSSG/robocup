...
class DribbleKick(behavior.Behavior):
	...
	def __init__(self,target):
		...
		self.add_state(DribbleKick.State.set_stance, behavior.Behavior.State.running)
...
# STATE FUNCTIONS:-
	...
	def on_enter_setstance(self):
		print("Entering SETSTANCE: ", self.turn)
		ballpos = self.kub.state.ballPos
		self.theta = angle_diff(ballpos, self.goal_point)
		self.go_at = getPointBehindTheBall(ballpos, self.theta, -2)

	def execute_setstance(self):
    	
		READY_TO_GOAL = self.ready_to_goal
		theta = self.theta
		rospy.wait_for_service('bsServer',)
		getState = rospy.ServiceProxy('bsServer',bsServer)
		...
		while not READY_TO_GOAL: 
			...
			if abs(vw)<1*MIN_w and READY_TO_GOAL==False:
				vw = 1*MIN_w*(1 if vw>0 else -1)
				
			if abs(vw) > MAX_w and READY_TO_GOAL==False:
				vw = (vw/abs(vw))*MAX_w
		...
		print("HO GYA GOAL KE TARAF ALIGN !! ---------------------------------------")
		

	def exit_setstance(self):
		print("EXIT SETSTANCE AT DBOX--------------------------")
		print("turndbox :",self.turndbox)
