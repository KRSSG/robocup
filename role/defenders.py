...
class defender(behavior.Behavior):
	...
	def should_intercept(self):
		...
		if not self.ball_moving():
			...
			our_time=dist(self.kub.state.homePos[self.defender_id],getPointBehindTheBall(ball,0))/MAX_BOT_SPEED
			if our_time > dist(self.kub.state.homePos[self.team_mate_id],getPointBehindTheBall(ball,0))/MAX_BOT_SPEED:
				return False
	...
	def execute_kick(self):
		...
		# opp_time=1000000
		# for opp_id, awayPos in enumerate(self.kub.state.awayPos):
		# 	if dist(awayPos,self.kub.state.ballPos)/MAX_BOT_SPEED<opp_time:
		# 		opp_time=dist(awayPos,self.kub.state.ballPos)/MAX_BOT_SPEED

		# our_time=dist(self.kub.state.homePos[self.kub.kubs_id],self.kub.state.ballPos)/MAX_BOT_SPEED
		# if opp_time-our_time<time_thresh:
		# 	angle=angle_diff(self.kub.state.ballPos,self.kicking_point)
		# 	self.kicking_point.x=self.kub.state.ballPos.x + HALF_FIELD_MAXX*math.cos(angle)
		# 	self.kicking_point.y=self.kub.state.ballPos.y + HALF_FIELD_MAXX*math.sin(angle)
		# else:
		# 	self.kicking_point=Vector2D(HALF_FIELD_MAXX,0)

		# if not vicinity_points(self.kub.state.ballPos,self.kub.get_pos(),thresh=2*BOT_RADIUS):
		# 	angle=angle_diff(self.kicking_point,self.kub.state.ballPos)
		# 	_GoToPoint_.init(self.kub,Vector2D(self.kub.state.ballPos),angle)
		# 	start_time = rospy.Time.now()
		# 	start_time = 1.0*start_time.secs + 1.0*start_time.nsecs/pow(10,9)  
		# 	generatingfunction = _GoToPoint_.execute(start_time,0.5*BOT_RADIUS)
		# 	for gf in generatingfunction:
		# 		self.kub,point = gf
		# 		if not vicinity_points(point,Vector2D(self.kub.state.ballPos),thresh=BOT_RADIUS*2.0):
		# 			break
		...
		# else:
		# 	self.kick_power=math.sqrt(dist(self.kicking_point,self.kub.state.ballPos)/6400.0)*5.0
		# 	angle=angle_diff(self.kub.state.ballPos,self.kicking_point)
		# 	_GoToPoint_.init(self.kub,Vector2D(self.kub.state.ballPos),angle)
		# 	start_time = rospy.Time.now()
		# 	start_time = 1.0*start_time.secs + 1.0*start_time.nsecs/pow(10,9)  
		# 	generatingfunction = _GoToPoint_.execute(start_time,0.25*BOT_RADIUS)
		# 	for gf in generatingfunction:
		# 		self.kub,point = gf
		# 		if not vicinity_points(point,Vector2D(self.kub.state.ballPos),thresh=BOT_RADIUS*2.0):
		# 			self.kub.kick(0)
		...
		# 		if not vicinity_points(self.kub.state.ballPos,self.kub.get_pos(),thresh=4*BOT_RADIUS):
		# 			break
		# 		if vicinity_points(self.kub.state.ballPos,self.kub.get_pos(),thresh=2*BOT_RADIUS):
		# 			self.kub.kick(self.kick_power)
		# 		else:
		# 			self.kub.kick(0)
