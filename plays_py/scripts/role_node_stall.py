...
from plays.pStall_node_function import assign_roles_stall
#from plays.pDefence_node_function import assign_roles_defence
...
class role_node(object):
    ...
    def tactic_instance(self, bot_id, tactic_id, params):
        ...
        assigned_role=assign_roles_stall(self.state)    #ATTACKER, PD, MD, MD, FORWARD
        if assigned_role==0:
    ...
    def bs_callback(self, data):
        ...
        receiver_bot_id[0] = assigned_role[1]
        if(receiver_bot_id[0] is not assigned_role[1]):
            tac_2 = self.tactic_instance(assigned_role[1], self.role_list[1][0], self.role_list[1][1])      #primary_defender
	        ...
	        tac_2.updateParams(self.state)
	        tac_2.execute(self.state, self.pub, int(assigned_role[0]))

        if(receiver_bot_id[0] is not assigned_role[2]):
            tac_3 = self.tactic_instance(assigned_role[2], self.role_list[2][0], self.role_list[2][1])      #middle_defender
            tac_3.updateParams(self.state)
            tac_3.execute(self.state, self.pub)
        else:
          tac_3 = self.tactic_instance(assigned_role[2], self.role_list[5][0], self.role_list[5][1])      #Receive
          tac_3.updateParams(self.state)
          tac_3.execute(self.state, self.pub, int(assigned_role[0]))

        if( receiver_bot_id[0] is not assigned_role[3]):
            tac_4 = self.tactic_instance(assigned_role[3], self.role_list[2][0], self.role_list[2][1])      #middle_defender
            tac_4.updateParams(self.state)
            tac_4.execute(self.state, self.pub)
        else:
          tac_4 = self.tactic_instance(assigned_role[3], self.role_list[5][0], self.role_list[5][1])      #Receive
          tac_4.updateParams(self.state)
          tac_4.execute(self.state, self.pub, int(assigned_role[0]))

        if( receiver_bot_id[0] is not assigned_role[4]):
            tac_5 = self.tactic_instance(assigned_role[4], self.role_list[3][0], self.role_list[3][1])      #Forward
            tac_5.updateParams(self.state)
            tac_5.execute(self.state, self.pub)
        else:
          tac_5 = self.tactic_instance(assigned_role[4], self.role_list[5][0], self.role_list[5][1])      #Receive
          tac_5.updateParams(self.state)
          tac_5.execute(self.state, self.pub, int(assigned_role[0]))


        tac_6 = self.tactic_instance(assigned_role[5], self.role_list[4][0], self.role_list[4][1])      #goalie
