...
namespace rrt
{
	...
	bool RRT<T>:: plan()
	{
		...
		int which=1;
		int check=0;
		std::cout<<"generating path for start= "<<startPoint.x<<","<<startPoint.y<<std::endl;
	...
	void RRT<T>::generatePath(Utils::Point<T> first,Utils::Point<T> last)
	{
		...
		int count=0;
		std::cout<<" in generatePath "<<std::endl;
		while(cur!=endPoint)
		{
		...
		count=0;
		std::cout<<" in generatePath cleared first loop, startPoint = "<<startPoint.x<<","<<startPoint.y<<std::endl;
		...
		std::cout<<" exiting generatePath "<<std::endl;
	}
