...
class TAttackerOpp(Tactic):
    def __init__(self, bot_id, state, param=None):
        super(TAttackerOpp, self).__init__( bot_id, state, param)
        self.bot_id = bot_id
    ...
    def Dist(self,a,state):
        a=a[0]
        return sqrt(pow((a.x-state.awayPos[self.bot_id].x),2)+ pow((a.y-(state.awayPos[self.bot_id].y)),2))
    def Dist2(self,a,state):
        return a[0]
    def Dist3(self,a,state):
        return fabs(a[0].x-state.awayPos[self.bot_id].x)+ fabs(a[0].y-int(state.awayPos[self.bot_id].y))
    ...
    def bisector_bot_wt(self,state):
        ...
        botPos = Vector2D(int(state.awayPos[self.bot_id].x), int(state.awayPos[self.bot_id].y))
        ...
        for opponent in opponents:
            ...
            ##print "my theta of :"+str(self.bot_id)+" is "+str((state.awayPos[self.bot_id].theta)/pi*180)
            angle=opponent[0].normalizeAngle(opponent[0].angle(botPos) - (state.awayPos[self.bot_id].theta))
        ...
        for i in xrange(len(state.awayPos)):
            ...
            dist=fabs(Vector2D(int(state.awayPos[i].x),int(state.awayPos[i].y)).dist(Vector2D(int(state.awayPos[self.bot_id].x),int(state.awayPos[self.bot_id].y))))
            angle_here=Vector2D(int(state.awayPos[i].x),int(state.awayPos[i].y)).angle(Vector2D(int(state.awayPos[self.bot_id].x),int(state.awayPos[self.bot_id].y))) - state.awayPos[self.bot_id].theta
            min_dist= np.inf
        ...
        self.bots_to_remove=[]
        # goal_angle_upper = botPos.normalizeAngle(Vector2D(int(self.UPPER_HALF.x), int(self.UPPER_HALF.y)).angle(botPos) - state.awayPos[self.bot_id].theta)#an2(UPPER_HALF.y-state.awayPos[self.bot_id].y, UPPER_HALF.x-state.awayPos[self.bot_id].x)
        # goal_angle_lower = botPos.normalizeAngle(Vector2D(int(self.LOWER_HALF.x), int(self.LOWER_HALF.y)).angle(botPos) - state.awayPos[self.bot_id].theta)
        ...
        # #goal_angle_lower = atan2(LOWER_HALF.y-state.awayPos[self.bot_id].y, LOWER_HALF.x-state.awayPos[self.bot_id].x)
        for i in xrange(len(state.awayPos)):
            if i==self.bot_id: continue
            myBotPos=Vector2D(int(state.awayPos[i].x),int(state.awayPos[i].y))
            angle=myBotPos.angle(botPos)-state.awayPos[self.bot_id].theta
            ...
            for angles in self.Opp_sorted_Angles:
                ...
                if (angle>=angles[0] and angle<=angles[1])  :
                    ...
                    dist_of_awayBot=Vector2D(int(state.homePos[angles[2]].x),int(state.homePos[angles[2]].y)).dist(botPos)
        ...
        for our_bot in xrange(len(state.awayPos)):
            if our_bot is self.bot_id:
                continue
            bot_angle = atan((state.awayPos[our_bot].y - botPos.y)/ (state.awayPos[our_bot].x - botPos.x))
    ...
    def bot_orient_wt(self,state):
        ...
        for our_bot in xrange(len(state.awayPos)):
            if our_bot is self.bot_id:
                continue
            angle_needed = no_use.normalizeAngle(state.awayPos[our_bot].theta-state.awayPos[self.bot_id].theta)
    ...
    def optimal_dist_wt(self,state):
        ...
        deviation = []
        AttackPos = Vector2D(int(state.awayPos[self.bot_id].x), int(state.awayPos[self.bot_id].y))
        for our_bot in xrange(len(state.awayPos)):
            if our_bot is self.bot_id:
                continue
            distance = fabs(Vector2D(int(state.awayPos[our_bot].x),int(state.awayPos[our_bot].y)).dist(Vector2D(int(state.awayPos[self.bot_id].x),int(state.awayPos[self.bot_id].y))))
    ...
    def calculate_score(self, state,BOTiD):
        ...
        weights = [0.35, 0.25, 0.15, 0.35, 0.15]
    ...
    def opponent_approachability_gwt(self, state, grid):
        ...
        for opp_id in xrange(len(state.homePos)):
            distance = fabs(Vector2D(int(state.homePos[opp_id].x), int(state.homePos[opp_id].y)).dist(grid.x, grid.y))
    ...
    def goal_possible(self,state):
        ...
        k1 = False
        k2 = False
        for away_bot in xrange(len(state.homePos)):
            
            if state.homePos[away_bot].x > state.awayPos[self.bot_id].x:
                ##print (state.homePos[away_bot].x, state.awayPos[self.bot_id].x, "chutiya.....")
                away_BOT = Vector2D (int(state.homePos[away_bot].x), int(state.homePos[away_bot].y))
                try:
                ...
                if bot_angle > goal_angle_mid and bot_angle < goal_angle_upper:
                    k2=True
                #print(k1,k2)
        for home_bot in xrange(len(state.awayPos)):
            
            if state.awayPos[home_bot].x > state.awayPos[self.bot_id].x:
                ##print (state.awayPos[home_bot].x, state.awayPos[self.bot_id].x, "chutiya.....")
                home_BOT = Vector2D (int(state.awayPos[home_bot].x), int(state.awayPos[home_bot].y))
                try:
    ...
    def execute(self, state, pub, receiver_bot_id):
        ...
        if not (state.ballPos.x < -HALF_FIELD_MAXX + DBOX_WIDTH  and  fabs(state.ballPos.y) < OUR_GOAL_MAXY*0.8) :
            if fabs(ballPos.dist(Vector2D(int(state.awayPos[self.bot_id].x),int(state.awayPos[self.bot_id].y)))>BOT_BALL_THRESH):
        ...
        if bool_goal[0] or goal_chck[0] :
            if bool_goal[0]:
                print("\n\n\n\n\n\n\n\n\n\n_______FUCK__GOALIE____\n\n\n\n\n\n\n\n\n\n")
            ...
            #     self.sParams.KickToPointP.power          = 7.0
            #     print("fuck bot ",self.bot_id)
            #     sKickToPointOpp.execute(self.sParams, state, self.bot_id, pub)
            #     flaggu=1
        ...
        #         print "_-_-_-_-_-_"+str(scores_of_bot[0][0])+"_-_-_-_-_-_"
        #         oh_my_bot=scores_of_bot[0][0]
        #     else:
        #         print "_-_-_-_-_-_"+str(scores_of_bot[1][0])+"_-_-_-_-_-_"
        #         oh_my_bot=scores_of_bot[1][0]
        # else:
        #      print "_-_-_-_-_-_"+str(scores_of_bot[0][0])+"_-_-_-_-_-_"
        best_bot=scores_of_bot[0][0]
        

        print("\n\n\n\n---------"+str(best_bot)+"---------\n\n\n\n")

          
        if possibility.isPassPossible(state, best_bot):
            #print("Passing.........................................................",self.bot_id,best_bot)
            if possibility.isReceivePossible(state, best_bot):
                #print("Receiving..............................................................",self.bot_id,best_bot)
                receiver_bot_id[0] = best_bot
                self.sParams.KickToPointP.x = state.awayPos[best_bot].x       
                self.sParams.KickToPointP.y = state.awayPos[best_bot].y
                self.sParams.KickToPointP.power         = 7
                sKickToPoint.execute(self.sParams, state, self.bot_id, pub)

        # import sGoToPointOppOpp
        # from random import randrange
        # for i in xrange(5):
        #     self.sParams.GoToPointP.x=((i*1000*pi+self.sParams.KickToPointP.x + state.awayPos[self.bot_id].x)/2)%3000
        #     self.sParams.GoToPointP.y=(randrange(-1,1,1)*randrange(-1000,1000,1))%3000
        #     sGoToPointOppOpp.execute(self.sParams,state,i,pub)
    ...
    def isComplete(self, state):
        ...
        if self.destination.dist(state.awayPos[self.bot_id]) < self.threshold:
            return True
