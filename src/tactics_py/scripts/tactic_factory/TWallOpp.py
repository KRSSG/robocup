...
BOT_GAP_THETA = asin(BOT_RADIUS*1.0/ R)
...
class TWall(Tactic):
    ...
    # def sol_with_circle(self,P,m,C,R):
    #     shiftx = C.x
    #     shifty = C.y
    #     tempC=Vector2D(0,0)
    #     x=P.x - shiftx
    #     y=P.y - shifty
    #     const = y - m*x
    #     X**2(1+m**2) + 2*m*const*X - R**2 + const**2=0
    #     delta = (2*m*const)**2 - 4*(const**2-R**2)*(1+m**2)



    #returns the intersection point of a line and circle
    def inter_circle_and_line(self, P, m, C, R):
        # x = P.x -HALF_FIELD_MAXX
        # y = P.y

        # const = -1*m*(HALF_FIELD_MAXX+P.x) + P.y
        # delta = (2*m*const)**2 - 4*(const**2-R**2)*(1+m**2)
        # # print delta
        # #print("delta",delta)
        # if delta < 0:
        ...
        # return True, P
        y = m*((HALF_FIELD_MAXX - DBOX_WIDTH) - P.x) + P.y
        if fabs(y) < 200:

            print("Solution",HALF_FIELD_MAXX-DBOX_WIDTH,y)
            return True, Vector2D((HALF_FIELD_MAXX-DBOX_WIDTH),int(y))
            ...
            delta = (2*m*const)**2 - 4*(const**2-R**2)*(1+m**2)
            if delta < 0:
                print("negative delta,no solution")
            ...
            X+=HALF_FIELD_MAXX
            Y+=SHIFT
            print("solution on curve",X,Y)
            return True,Vector2D(int(X),int(Y))
    ...
    def find_relative_point(self, sol, idd):
        ...
        if not idd :
            ...
            print("solution with minus theta")
            # return Vector2D(int(2800),int(250))
        else:
            theta_t = BOT_GAP_THETA
            print("solution with plus theta")
            # return Vector2D(int(2800),int(20))
        ...
        # X[1] += C.y
        # if fabs(X[1]) < 340 :
        #     X[0] -= X[0] - HALF_FIELD_MAXX + DBOX_WIDTH
        #     X[0] += BOT_RADIUS
        # #print("iddddd ",X)
        # print("given solution",sol.x,sol.y)
        # print("find solution",X)
        # print(X)

        if fabs(sol.y)<200:
            if not idd:
                return Vector2D(int(sol.x),int(sol.y - 2*BOT_RADIUS))
            else:
                return Vector2D(int(sol.x),int(sol.y + 2*BOT_RADIUS))
        else:
            if not idd :
                ...
                print("solution with plus theta")
            if sol.y>200:
                finaltheta = sol.angle(Vector2D(HALF_FIELD_MAXX,200)) + BOT_GAP_THETA
                Y = 200 + 500*sin(finaltheta)
                X = HALF_FIELD_MAXX + 500*cos(finaltheta)
        ...
        # maintheta = Vector2D(int(HALF_FIELD_MAXX),0).angle(sol)
        # theta = maintheta + theta_t
        # X = int(HALF_FIELD_MAXX) + R*cos(theta)
        # Y = R*sin(theta)
        # print("point",X,Y)
    ...
    def execute(self, state , pub, wall ):
        ...
        if state.ballPos.x < HALF_FIELD_MAXX and fabs(state.ballPos.y) < HALF_FIELD_MAXY or 1:
            ...
            if min_dist < BOT_BALL_DIST_THRESH :
                ...
                ballPos=Vector2D(int(state.ballPos.x),int(state.ballPos.y))
                threat_bot_theta = ballPos.angle(Vector2D(int(state.awayPos[threat_bot].x),int(state.awayPos[threat_bot].y)))
                # threat_bot_pos = Vector2D(int(state.awayPos[p_threat_id].x), int(state.awayPos[p_threat_id].y))
                print(threat_bot_theta)
                bool_sol, sol = self.inter_circle_and_line(threat_bot_pos,threat_bot_theta , C, R)
                #print("threat1",threat_bot,bool_sol)
                if not bool_sol:
                    print("now trying with center angle",threat_bot)
                    threat_bot_theta = C.angle(threat_bot_pos)
                    # threat_bot_theta=atan2(- threat_bot_pos.y + C.y, - threat_bot_pos.x + C.x)
                    print(threat_bot_pos.x,threat_bot_pos.y)
                    print(C.x,C.y)
                    print(threat_bot_theta)
                    print("hereee",atan2(1200,2000))
                    bool_sol, sol = self.inter_circle_and_line(threat_bot_pos,threat_bot_theta , C, R) 
                else:
                ...
                print("angle of ball (in twall opp)",angle)
                threat_bot_theta=angle
                bool_sol, sol = self.inter_circle_and_line(threat_bot_pos, threat_bot_theta, C, R)
                ...
                if not bool_sol:
                    ...
                    threat_pointing_goal = True
            if  self.wall_bots[0] is self.bot_id:
                if state.homePos[self.wall_bots[0]].y < state.homePos[self.wall_bots[1]].y:
                    target_point = self.find_relative_point(sol,1)
                else:
                    target_point = self.find_relative_point(sol,0)
                #print("dtarget_pt "+str(0),target_point.x, target_point.y,threat_bot_theta)
            else:
                if state.homePos[self.wall_bots[0]].y < state.homePos[self.wall_bots[1]].y:
                    target_point = self.find_relative_point(sol,0)
                else:
                    target_point = self.find_relative_point(sol,1)
                #print("dtarget_pt "+str(1),target_point.x, target_point.y,threat_bot_theta)
            #target_point = self.find_relative_point(sol,1)

            
            #print(threat_bot_pos.x, threat_bot_pos.y)
            # self.sParams.GoToPointP.x = target_point.x
            # self.sParams.GoToPointP.y = target_point.y
            # self.sParams.GoToPointP.finalslope = threat_bot_theta
            # sGoToPoint.execute(self.sParams, state, self.bot_id, pub)
            ######################################        put move in arc here   #####################################
            print("checker",sol.x,sol.y)
            print("wall bots",self.wall_bots[0],self.wall_bots[1])
            print("botid",self.bot_id)

            print("sendign bot to point")
            print(self.bot_id,target_point.x,target_point.y)
            print("current position",state.homePos[self.bot_id].x,state.homePos[self.bot_id].y)
            self.sParams.GoToPointP.x =  target_point.x
            self.sParams.GoToPointP.y =  target_point.y
            # self.sParams.GoToPointP.x = sol.x
            # self.sParams.GoToPointP.y = sol.y
            ...
            print("Position of wall",self.sParams.GoToPointP.x,self.sParams.GoToPointP.y)
            sGoToPoint.execute(self.sParams, state, self.bot_id, pub)
        
        if self.bot_id is self.wall_bots[0]:
            d1 = ballPos.dist(botPos)   
            d2 = ballPos.dist(Vector2D(int(state.homePos[self.wall_bots[1]].x), int(state.homePos[self.wall_bots[1]].y)))
        else:
            d1 = ballPos.dist(botPos)   
            d2 = ballPos.dist(Vector2D(int(state.homePos[self.wall_bots[0]].x), int(state.homePos[self.wall_bots[0]].y)))
