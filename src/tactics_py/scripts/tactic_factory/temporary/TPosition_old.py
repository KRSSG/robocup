...
class TPosition(Tactic):
    ...
    def kick_goal_wt(self,state):
        ...
        for our_bot in xrange(len(state.homePos)):
            if our_bot is self.bot_id:
                continue
            home_bot = Vector2D (int(state.homePos[our_bot].x), int(state.homePos[our_bot].y))

            #print ("maha chutiya ",home_bot.x - botPos.x, botPos.x)
            try:
                bot_angle = atan((home_bot.y - botPos.y)*1.0/ (home_bot.x - botPos.x))
            except:
                bot_angle = pi/2
            #print("ANGLES    ",bot_angle, goal_angle_upper, goal_angle_lower, our_bot)

            if bot_angle < goal_angle_upper and bot_angle > goal_angle_lower and home_bot.x > 0:
                dist = home_bot.dist(Vector2D(HALF_FIELD_MAXX,0))
                bot.append(our_bot)
                normalizeDistance = KICK_DISTANCE_CONST * exp(-1*dist/HALF_FIELD_MAXX)
                our_bot_kick.append([our_bot, normalizeDistance])
            else:
                our_bot_kick.append([our_bot, 0])
        #print ("lauda wala bots      ",bot)
    ...
    def calculate_score(self, state):
        ...
        weights = [0.35, 0.25, 0.15, 0.05, 0.15, 0.5]
        ...
        score_bot_kick = self.kick_goal_wt(state)
        ...
        for i in xrange(len(state.homePos)):
            ...
            (score_bot_kick[our_bot][1]*weights[5])
    ...
    def execute(self, state, pub):
        ...
        for i in state.homePos:
            ...
            if dist<DistancesFromBot:
                ...
                self.bot_id=count
            count+=1
        goal_chck = self.goal_posible(state)
        import sKickToPoint
        import sGoToBall
        if fabs(ballPos.dist(Vector2D(int(state.homePos[self.bot_id].x),int(state.homePos[self.bot_id].y)))>DRIBBLER_BALL_THRESH):
        ...
        if scores_of_bot[0][1] - scores_of_bot[1][1] < PASSING_PARAMETER and (scores_of_bot[0][1] and scores_of_bot[1][1]):
            self.bot_id = scores_of_bot[0][0]
            scores_of_bot_primary = self.calculate_score(state)
            ...
            self.bot_id =scores_of_bot[1][0]
            scores_of_bot_secondary = self.calculate_score(state)
