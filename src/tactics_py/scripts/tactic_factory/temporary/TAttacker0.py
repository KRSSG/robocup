...
class TAttacker(Tactic):
    ...
    def bot_orient_wt(self,state):
        ...
        for our_bot in xrange(len(state.homePos)):
            if our_bot is self.bot_id:
                continue
            angle_needed = no_use.normalizeAngle(fabs(state.homePos[our_bot].theta-state.homePos[self.bot_id].theta)-pi )
            # deviation.append([our_bot, normalizeDistance])
            
            normalizedAngle  = BOT_ORIENT_CONST * exp(-1*angle_needed/ pi)
    ...
    def execute(self, state, pub, receiver_bot_id):
        ...
        if not flaggu :    
            if possibility.isPassPossible(state, best_bot):
                ...
                if possibility.isReceivePossible(state, best_bot):
                    ...
                    self.sParams.KickToPointP.x = state.homePos[best_bot].x       
                    self.sParams.KickToPointP.y = state.homePos[best_bot].y
        ...
        for i in xrange(5):
            self.sParams.GoToPointP.x=((i*1000*pi+self.sParams.KickToPointP.x + state.homePos[self.bot_id].x)/2)%3000
            self.sParams.GoToPointP.y=(randrange(-1,1,1)*randrange(-1000,1000,1))%3000
            sGoToPointOpp.execute(self.sParams,state,i,pub)
