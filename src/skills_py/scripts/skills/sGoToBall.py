...
def execute(param,state,bot_id, pub, dribbler=False):
    ...
    #print (bot_id)#,"in GoToBall dist=",dist)
    maxDisToTurn = dist -  BOT_BALL_THRESH
    ...
    speed= maxDisToTurn*MAX_BOT_SPEED/(HALF_FIELD_MAXX)

    if (speed)< 32*MIN_BOT_SPEED:
        speed=32*MIN_BOT_SPEED

    vec = Vector2D()
    motionAngle = nextWP.angle(botPos)
    theta  = motionAngle - state.homePos[bot_id].theta

    # print "BOT_BALL_THRESH="+str(BOT_BALL_THRESH)
    # print "DRIBBLER_BALL_THRESH="+str(DRIBBLER_BALL_THRESH)
    # print "dist/BOT_BALL_THRESH="+str(dist/BOT_BALL_THRESH)
    # print "dist/DRIBBLER_BALL_THRESH="+str(dist/DRIBBLER_BALL_THRESH)
