...
class CoordinatedPass(composite_behavior.CompositeBehavior):
    ...
    def __init__(self,
        ...
        super(CoordinatedPass,self).__init__()
        ...
        self.receiver = skillreceiver
        self.kicker = skillkicker
        ...
        self.add_transition(
            ...
            lambda: self.subbehavior_with_name('receiver').state == self.skillreceiver.State.aligned,
    ...
    def on_enter_preparing(self):
        ...
        goal = self.receive_point
        kicker_pos = Vector2D(self.skillkicker.kub.get_pos())
        angle = kicker_pos.angle(goal)
        self.skillkicker.add_theta(theta=angle)
        self.add_subbehavior(self.skillkicker,"kicker")



        self.skillreceiver = pass_receive.PassReceive()
        self.skillreceiver.as_graphviz()
        self.skillreceiver.write_diagram_png()
        # self.skillreceiver.spin_cb()    
        # self.receiver.update_state(state)
        self.skillreceiver.add_kub(self.receiver)
        self.skillreceiver.receive_point = self.receive_point
        self.add_subbehavior(self.skillreceiver,'receiver')
        ...
        # kicker.add_theta(theta=normalize_angle(pi+atan2(self.kub.state.ballPos.y,self.kub.state.ballPos.y-3000)))
        # self.add_subbehavior(kicker,'fetch')

        # kicker = self.skillkicker[0]
        # kicker.target = self.receive_point
        # kickpower = (main.ball().pos - self.receive_point).mag() / 8
        # kicker.kick_power = 7.0
    ...
    def execute_preparing(self):
        print("Pass in execute preparing")
        pass
        # kicker = self.subbehavior_with_name('kicker')
        ...
        #     kicker_pos = Vector2D(self.kicker.get_pos())
        #     kick_line = Line(kicker_pos, angle=self.kicker.pos.theta)
        #     if (self._last_unsteady_time != None and
        #             time.time() - self._last_unsteady_time > 0.75 and
        #             kick_line.distance_from_point(self.receive_point) < BOT_RADIUS):
        #         # renegotiate receive_point
        #         logging.info("Pass renegotiated RCV PT")
        #         self.receive_point = kick_line.projection_on_line(self.receive_point)
        #         self._has_renegotiated_receive_point = True

    def on_enter_kicking(self):
        print("pass in on enter kicking")
        pass
        # self.subbehavior_with_name('kicker').enable_kick = True
