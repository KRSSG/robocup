...
if __name__ == '__main__':
    ...
    plt.plot(np.arange(0, len(vel_rect)),vel_rect[:,0])
    plt.xlabel('number of measurments')
    plt.ylabel('x-velocity')
    plt.title('vel_x')
    plt.figure()
    plt.plot(np.arange(0, len(vel_rect)),vel_rect[:,1])
    plt.xlabel('number of measurments')
    plt.ylabel('y-velocity')
    plt.title('vel_y')
    plt.figure()
    plt.plot(np.arange(0, len(arr_rect)),arr_rect[:,0])
    plt.xlabel('number of measurments')
    plt.ylabel('x')
    plt.title('pos_x')
    plt.figure()
    plt.plot(np.arange(0, len(arr_rect)),arr_rect[:,1])
    plt.xlabel('number of measurments')
    plt.ylabel('y')
    plt.title('pos_y')
    plt.show()
